...
export function Snapshots() {
  ...
  if (needsTenant) {
    return <div className="text-center py-12 text-amber-600">Selecione um tenant para gerenciar snapshots.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Volume Snapshots</h1>
          <p className="text-gray-500">{snapshots.length} volume snapshots</p>
        </div>
        <div className="flex gap-3">
          <RefreshButton onRefresh={() => refetch()} isFetching={isFetching} dataUpdatedAt={dataUpdatedAt} />
          <button onClick={() => setCreateModal(true)} className="btn-primary">
            <Plus size={18} /> Criar Snapshot
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          ...
          <div className="col-span-full text-center py-12">
            ...
            <p className="text-gray-500">Nenhum snapshot encontrado</p>
          </div>
        ) : (
          filtered.map((snap) => (
            <div key={snap.id} className="bg-white dark:bg-dark-100 rounded-xl border p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Volume: {snap.volume_id.slice(0, 8)}...</p>
                </div>
              </div>
      ...
      <Modal isOpen={createModal} onClose={() => setCreateModal(false)} title="Criar Snapshot">
        <form
          onSubmit={(e) => {
            e.preventDefault();
